...
function main () {
    ...
    numberOfDraws.forEach((handSize) => { 
        const { probability, benchmark, iterations, total } = play(givenCard, handSize);
        console.log(`The probability of not passing in blackjack with a ${givenCard} choosing ${handSize} cards is of ${probability} %.`);
        console.log(`TIME: ${benchmark} ms`);
...
function getParameters () {
    ...
    if (process.argv.includes("-intense")) {
        ...
        if (process.argv.includes("-nopick")) {
            return { givenCard: 0, numberOfDraws: [...Array(10).keys()].slice(1) }
        }
        return { givenCard: parseInt(process.argv[0]), numberOfDraws: [...Array(10).keys()].slice(1) }
    } 
    if (process.argv.includes("-nopick")) {
...
function play(givenCard, handSize) {
    ...
    const iterator = new Iterator(handSize, play.deck, 21 - givenCard);
